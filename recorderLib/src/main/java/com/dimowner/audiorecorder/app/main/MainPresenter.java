...
public class MainPresenter implements MainContract.UserActionsListener {
	...
	private float dpPerSecond = CommonConstants.SHORT_RECORD_DP_PER_SECOND;
			...
			appRecorderCallback = new AppRecorderCallback() {
				...
				public void onRecordingStopped(final File file, final Record rec) {
					...
					if (deleteRecord) {
						...
						dpPerSecond = ARHandler.Companion.getDpPerSecond((float) songDuration / 1000000f);
						if (view != null) {
						...
						CommonAndroidUtils.runOnUIThread(new Runnable() {
							@Override public void run() {
								if (view != null) {
									...
									if (duration > 0) {
										view.onPlayProgress(mills, CommonAndroidUtils.convertMillsToPx(mills,
												CommonAndroidUtils.dpToPx(dpPerSecond)), (int) (1000 * mills / duration));
									}
								}
							}});
					}
		...
		if (audioPlayer.isPlaying()) {
			...
			if (view != null) {
				...
				if (duration > 0) {
					...
					view.onPlayProgress(playProgressMills, CommonAndroidUtils.convertMillsToPx(playProgressMills,
							CommonAndroidUtils.dpToPx(dpPerSecond)), (int) (1000 * playProgressMills / duration));
				}
				view.showPlayPause();
			}
		} else {
	...
	public void seekPlayback(int px) {
		audioPlayer.seek(CommonAndroidUtils.convertPxToMills(px, CommonAndroidUtils.dpToPx(dpPerSecond)));
	}
	...
	public void renameRecord(final long id, final String newName, final String extension, final boolean needDecode) {
		...
		loadingTasks.postRunnable(new Runnable() {
			@Override public void run() {
				...
				if (record != null) {
					String nameWithExt = name + CommonConstants.EXTENSION_SEPARATOR + extension;
								...
								CommonAndroidUtils.runOnUIThread(new Runnable() {
									...
									public void run() {
										if (view != null) {
											view.hideProgress();
											view.showName(name);
											if (needDecode && record.getDuration()/1000 < CommonConstants.DECODE_DURATION) {
	...
	public void decodeRecord(long id) {
		loadingTasks.postRunnable(new Runnable() {
			...
			public void run() {
				...
				if (rec != null && rec.getDuration()/1000 < CommonConstants.DECODE_DURATION) {
	...
	private void updateInformation(String format, int sampleRate, long size) {
		if (format.equals(CommonConstants.FORMAT_3GP)) {
			if (view != null) {
				view.showInformation(settingsMapper.formatSize(size) + CommonConstants.SEPARATOR
						+ settingsMapper.convertFormatsToString(format) + CommonConstants.SEPARATOR
			...
			if (view != null) {
				switch (format) {
					...
					default:
						view.showInformation(settingsMapper.formatSize(size) + CommonConstants.SEPARATOR
								+ format + CommonConstants.SEPARATOR
								+ settingsMapper.convertSampleRateToString(sampleRate)
						);
				}
			}
		...
		if (rec != null) {
			...
			recordingsTasks.postRunnable(new Runnable() {
				...
				public void run() {
					...
					dpPerSecond = CommonConstants.SHORT_RECORD_DP_PER_SECOND;
					CommonAndroidUtils.runOnUIThread(new Runnable() {
						@Override
						public void run() {
							if (view != null) {
				...
				try {
					...
					if (FileUtil.copyFile(fileDescriptor, newFile)) {
						...
						Record r = new Record(
								...
								false,
								false,
								new int[ARHandler.Companion.getLongWaveformSampleCount()]);
						...
						if (rec != null) {
							...
							if (rec.getDuration()/1000 < CommonConstants.DECODE_DURATION) {
								appRecorder.decodeRecordWaveform(rec);
							}
						}
					}
