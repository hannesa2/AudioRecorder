...
public abstract class DataSource<T> {
    ...
    public T insertItem(T item) {
        ...
        if (values != null) {
            ...
            Timber.d("Insert into " + tableName + " id = " + insertId);
            return getItem(insertId);
        } else {
            Timber.e("Unable to write empty item!");
            return null;
        }
    }
    ...
    public int deleteItem(int id) {
        Timber.d(tableName + " deleted ID = " + id);
    ...
    public int updateItem(T item) {
        ...
        if (values != null && values.containsKey(SQLiteHelper.COLUMN_ID)) {
            ...
            Timber.d("Updated records count = " + n);
            return n;
        } else {
            Timber.e("Unable to update empty item!");
            return 0;
        }
    }
    ...
    public ArrayList<T> getRecords(int page) {
        Cursor cursor = queryLocal("SELECT * FROM " + tableName
                ...
                + " LIMIT " + CommonConstants.DEFAULT_PER_PAGE
                + " OFFSET " + (page - 1) * CommonConstants.DEFAULT_PER_PAGE);
        return convertCursor(cursor);
    }
    ...
    protected Cursor queryLocal(String query) {
        Timber.d("queryLocal: " + query);
        Cursor c = db.rawQuery(query, null);
        if (ARHandler.Companion.getDebugBuild()) {
